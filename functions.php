...
function buildBaseString(string $baseURI, string $method, array $params): string
{
    $r = array();
    ksort($params);
...
function buildAuthorizationHeader($oauth): string
{
...
function returndata(string $user): array
{
    ...
    $header = [buildAuthorizationHeader($oauth), 'Expect:'];
    $options = [
    ...
    foreach ($tweets as $tweet) {
        ...
        if (is_null($website = $tweet["user"]["url"])) {
            $website = null;
        }

        if (is_null($tweet["user"]["entities"]["url"]['urls'][0]['expanded_url'])) {
            $website_link = null;
        } else {
            $website_link = $tweet["user"]["entities"]["url"]['urls'][0]['expanded_url'];
        }

    }

    return [
        'id' => $userid, 'userName' => $username, 'name' => $name, 'location' => $location, 'description' => $description, 'follower' => $followers,
